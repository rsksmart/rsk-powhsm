...
int cx_ecfp_generate_pair(
    ...
    secp256k1_ec_pubkey_serialize(sp_ctx, pubkey->W, &pubkey_size, &sp_pubkey, SECP256K1_EC_UNCOMPRESSED);
...
int cx_ecdsa_sign(
    ...
    secp256k1_ecdsa_signature_serialize_der(sp_ctx, sig, &sig_serialized_size, &sp_sig);
...
size_t hsmsim_helper_getpubkey_compressed(const unsigned char* key, unsigned char* dest, size_t dest_size) {
    ...
    secp256k1_ec_pubkey_serialize(sp_ctx, dest, &dest_size, &pubkey, SECP256K1_EC_COMPRESSED);
    return dest_size;
}