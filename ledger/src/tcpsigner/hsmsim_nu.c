...
void hsmsim_set_network_upgrade(uint32_t block_number, uint8_t* dst_network_upgrade) {
    ...
    for (int i = 0; i < sizeof(network_upgrade_activations)/sizeof(network_upgrade_activations[0]); i++) {
        if (block_number >= network_upgrade_activations[i].activation_bn && 
            network_upgrade_activations[i].activation_bn >= current.activation_bn) {

            current.activation_bn = network_upgrade_activations[i].activation_bn;
            current.network_upgrade = network_upgrade_activations[i].network_upgrade;
        }
