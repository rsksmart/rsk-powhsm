...
while (true) {
    ...
    if ((APDU_OP() & 0xF) != P1_PATH && (APDU_OP() & 0xF) != P1_BTC &&
    ...
    if ((APDU_OP() & 0xF) == P1_PATH) {
        if ((rx != DATA + PATHLEN + INPUTINDEXLEN) &&
            ...
            THROW(0x6A87); // Wrong buffer size, has to be either 28
                           // (DATA+PATHLEN+INPUTINDEXLEN) or 56
                           // (DATA+PATHLEN+HASHEN)
        SAFE_MEMMOVE(path,
                     sizeof(path),
                     APDU_DATA_PTR + 1,
        ...
        // If path doesn't require authorization, go directly do the signing
        // state
    ...
    if (APDU_OP() & P1_BTC) {
        ...
        if ((state != S_TX_REMAINING &&
             (rx - DATA) != tx_ctx.expectedRXBytes) ||
    ...
    if (APDU_OP() & P1_RECEIPT) {
        ...
        case S_RLP_FIELD:
            ...
            if ((rlp_ctx.listLevel == 3) && (rlp_ctx.fieldCount == 1)) {
                ...
                if (rx != CONTRACTADDRESS_LEN + DATA)
                    THROW(0x6A87);
                if (!memcmp(
                        APDU_DATA_PTR, ContractAddress, CONTRACTADDRESS_LEN))
            ...
            if ((rlp_ctx.listLevel == 4) &&
                ...
                if (!memcmp(
                        ...
                        APDU_DATA_PTR,
                        sizeof(
                            tx_ctx.TXHashBuf))) { // Matching TX found, Contract
                                                  // is valid and Receipt
        ...
        case S_RLP_FINISH:
            ...
            if (rx - DATA == RLP_MAX_TRANSFER) { // Data still remains
                ...
                LOG_HEX("LAST CHUNK, RLP Keccak256: ",
                        ReceiptHashBuf,
                        sizeof(ReceiptHashBuf));
                SET_APDU_FOR_SIGN();
                SET_APDU_OP(P1_MERKLEPROOF);
    ...
    //---------------------- Merkle Proof Parser --------------------------
        ...
        case S_MP_NODE_REMAINING:
            ...
            break;
        case S_SIGN_MESSAGE: {
            // Matching TX found, Contract is valid, Receipt Signature is valid
            // and Merkle Tree passes al verifications. Sign the signatureHash
        ...
        // CMD_FINISHED state uses the whole buffer, no expectedRXbytes are
        // needed.
...
continue_sign_loop:
    ...
    if (APDU_CMD() == INS_SIGN && APDU_TXLEN() == 0 &&
        state != S_CMD_FINISHED) {
        rx = 3;
    } else {
        break;
