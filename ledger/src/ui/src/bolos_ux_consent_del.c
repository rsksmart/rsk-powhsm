...
const bagl_element_t screen_consent_del_elements[] = {
    // erase
    {{BAGL_RECTANGLE,
      0x00,
      0,
...
const bagl_element_t *screen_consent_del_before_element_display_callback(
    ...
    case 0x11:
        screen_prepare_masked_icon(
            G_bolos_ux_context.parameters.u.appdel.appentry.icon,
            G_bolos_ux_context.parameters.u.appdel.appentry.icon_length);
        break;

    case 0x31:
        strcpy(
            G_bolos_ux_context.string_buffer,
            (const char *)G_bolos_ux_context.parameters.u.appdel.appentry.name);
        break;
