...
const bagl_element_t screen_consent_setup_customca_elements[] = {
    // erase
    {{BAGL_RECTANGLE,
      0x00,
      0,
...
screen_consent_setup_customca_before_element_display_callback(
    ...
    case 0x31:
        SPRINTF(
            ...
            "%.*s",
            MIN(CUSTOMCA_MAXLEN,
                strlen((char *)G_bolos_ux_context.parameters.u.setup_customca
                           .name)),
            (const char *)G_bolos_ux_context.parameters.u.setup_customca.name);
        break;

    case 0x32:
        screen_hex_identifier_string_buffer(
            G_bolos_ux_context.parameters.u.setup_customca.public.W,
            G_bolos_ux_context.parameters.u.setup_customca.public.W_len);
        break;
    }
...
void screen_consent_setup_customca_init(void) {
    ...
    G_bolos_ux_context.screen_stack[0].element_arrays[0].element_array =
        screen_consent_setup_customca_elements;
    G_bolos_ux_context.screen_stack[0].element_arrays[0].element_array_count =
        ARRAYLEN(screen_consent_setup_customca_elements);
