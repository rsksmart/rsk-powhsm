...
unsigned int screen_settings_change_pin_1_2_pin_button(
...
unsigned int screen_settings_change_pin_3_pin_button(
    unsigned int button_mask, unsigned int button_mask_counter) {
    UNUSED(button_mask_counter);
    switch (button_mask) {
    case BUTTON_EVT_RELEASED | BUTTON_LEFT | BUTTON_RIGHT:
...
unsigned int screen_settings_change_pin_1_2_pin_entered(
    ...
    if (G_bolos_ux_context.onboarding_index == 0) {
        ...
        if (G_bolos_ux_context.words_buffer[0] != pin_length ||
            os_memcmp(pin_buffer,
                      G_bolos_ux_context.words_buffer + 1,
                      pin_length) != 0) {
