...
class TestSync(TestCase):
    ...
    def setUp(self):
        ...
        self.minimum_cumulative_difficulty = self.DIFFICULTY_MULTIPLIER*self.checkpoint_header.difficulty
        self.blockchain_state = BlockchainState.checkpoint(self.checkpoint_header.hash)
        self.blockchain_state.minimum_cumulative_difficulty = self.minimum_cumulative_difficulty
        # We don't need the wallet or log emitter address for these tests
        self.protocol = HSM2ProtocolSimulator(
            wallet=None, blockchain_state=self.blockchain_state,
            log_emitter_address="00"*20, network_parameters=self.netparams,
    ...
    def test_incremental_sync(self):
        ...
        for i in range(CHUNKS):
            ...
            self.assertFalse(state["updating"]["already_validated"])
            sent_best_block = sent_best_block or end is None or \
                              expected_best_index in range(start, end)
            if not is_last_chunk:
                ...
                self.assertEqual(state["updating"]["next_expected_block"], \
