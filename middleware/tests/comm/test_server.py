...
from comm.server import TCPServer, TCPServerError, _RequestHandler, \
    RequestHandlerError, RequestHandlerShutdown
...
class TestTCPServer(TestCase):
    ...
    def assert_server_setup_ok(self, TCPServerMock):
        self.assertEqual(
            TCPServerMock.call_args_list,
            [call(('a-host', 1234), ANY)])
...
class TestTCPServerRequestHandler(TestCase):
    ...
    def test_handles_ok(self):
        self.assertEqual(self.RequestHandlerMock.call_args_list, [call(self.protocol, self.server.logger)])
        self.assertEqual(self.request_handler.handle.call_args_list, \
...
class TestRequestHandler(TestCase):
    ...
    def test_handle_ok(self):
        ...
        self.assertEqual(
            ...
            [call({"this-is": "legal", "json": "format"})])
        self.assertEqual(
            self.wfile.write.call_args_list,
            [call('{"the": "result", "this": "is"}'.encode('utf-8')), call("\n".encode("utf-8"))])
    ...
    def test_handle_json_error(self):
        ...
        self.assertEqual(
            self.wfile.write.call_args_list,
            [call('{"a": "bad", "format": "error"}'.encode('utf-8')), call("\n".encode("utf-8"))])

    def test_handle_notimplemented_error(self):
        self.mock_request('{"another": "valid", "json": "request"}')
        self.protocol.handle_request.side_effect = NotImplementedError("method not implemented")

        self.do_request()

        self.assertEqual(
            self.protocol.handle_request.call_args_list,
            [call({"another": "valid", "json": "request"})])
        self.assertEqual(
            self.wfile.write.call_args_list,
            [call('{}'.encode('utf-8')), call("\n".encode("utf-8"))])
    ...
    def test_handle_protocol_error(self):
        ...
        self.assertEqual(
            self.wfile.write.call_args_list,
            [call('{"an": "unknown", "e": "rror"}'.encode('utf-8')), call("\n".encode("utf-8"))])

    def test_handle_protocol_shutdown(self):
        self.mock_request('{"another": "valid", "json": "request"}')
        self.protocol.handle_request.side_effect = HSM2ProtocolInterrupt("protocol interrupt")
    ...
    def test_handle_invalid_encoding(self):
        ...
        self.assertEqual(
            self.wfile.write.call_args_list,
            [call('{"a": "bad", "encoding": "error"}'.encode('utf-8')), call("\n".encode("utf-8"))])
