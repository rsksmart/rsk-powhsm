...
class TestHSM1Protocol(TestCase):
    ...
    def test_invalid_request_no_command(self):
        self.assertEqual(
            self.protocol.handle_request({"version": 1}),
            {"errorcode": -2})
    ...
    def test_invalid_command(self):
        self.assertEqual(
            self.protocol.handle_request({"command": "invalid", "version": 1}),
            {"errorcode": -2})
    ...
    def test_getpubkey_keyId_presence(self):
        self.assertEqual(
            self.protocol.handle_request({"command": "getPubKey", "version": 1}),
            {"errorcode": -2})

    def test_getpubkey_keyId_notastring(self):
        self.assertEqual(
            self.protocol.handle_request({"command": "getPubKey", "version": 1, "keyId": 123}),
            {"errorcode": -2})

    def test_getpubkey_keyId_invalid(self):
        self.assertEqual(
            self.protocol.handle_request({"command": "getPubKey", "version": 1, "keyId": "not-a-key-id"}),
            {"errorcode": -2})

    def test_getpubkey_notimplemented(self):
        with self.assertRaises(NotImplementedError):
            self.protocol.handle_request({"command": "getPubKey", "version": 1, "keyId": "m/0/0/0/0/0"})

    def test_sign_keyId_presence(self):
        self.assertEqual(
            self.protocol.handle_request({"version": 1, "command": "sign"}),
            {"errorcode": -2})

    def test_sign_keyId_not_a_string(self):
        self.assertEqual(
            self.protocol.handle_request({"version": 1, "command": "sign", "keyId": 1234}),
            {"errorcode": -2})

    def test_sign_keyId_invalid(self):
        self.assertEqual(
            self.protocol.handle_request({"version": 1, "command": "sign", "keyId": "not-a-key-id"}),
            {"errorcode": -2})

    def test_sign_message_presence(self):
        self.assertEqual(
            self.protocol.handle_request({"version": 1, "command": "sign", "keyId": "m/0/0/0/0/0"}),
            {"errorcode": -2})
