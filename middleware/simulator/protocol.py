...
from .authorization import (
    ...
    is_authorized_signing_path,
    is_auth_requiring_path,
)
from comm.utils import normalize_hex_string, is_hex_string_of_length


class HSM2ProtocolSimulator(HSM2Protocol):
    def __init__(self, wallet, blockchain_state, log_emitter_address, network_parameters,
                 speed_bps):
        super().__init__()
        self._wallet = wallet
    ...
    def _advance_blockchain(self, request):
        return self.__do_block_operation(
            request,
            self._blockchain_state.advance,
            "advancing blockchain",
            mm_is_mandatory=True,
        )
    ...
    def _update_ancestor_block(self, request):
        return self.__do_block_operation(
            request,
            self._blockchain_state.update_ancestor,
            "updating the ancestor block",
            mm_is_mandatory=False,
        )
    ...
    def __do_block_operation(self, request, processor, operation_name, mm_is_mandatory):
        ...
        try:
            blocks = list(
                map(
                    lambda rb: RskBlockHeader(
                        rb, self._network_parameters, mm_is_mandatory=mm_is_mandatory),
                    request["blocks"],
                ))
