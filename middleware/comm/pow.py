...
# Taken from rskj
# https://github.com/rsksmart/rskj/blob/32dbe675b2e22f1c804e2d1c8a76b37e2020f05b/rskj-core/src/main/java/co/rsk/util/DifficultyUtils.java#L32
...
# Based on rskj.
# See https://github.com/rsksmart/rskj/blob/master/rskj-core/src/main/java/co/rsk/validators/ProofOfWorkRule.java#L162
# for details.
# *** IMPORTANT ***:
...
def coinbase_tx_extract_merge_mining_hash(tx_hex):
    try:
        ...
        if last_tag_pos == -1:
            message = "Couldn't find RSK tag '%s' in tail '%s'" % (_RSK_TAG.hex(), tx_tail.hex())
            _logger.info(message)
            raise ValueError(message)

        if last_tag_pos >= _MAX_RSK_TAG_POSITION:
            message = "RSK tag '%s' position in tail '%s' is bigger than expected (%d)" % (_RSK_TAG.hex(), tx_tail.hex(), _MAX_RSK_TAG_POSITION)
            _logger.info(message)
            raise ValueError(message)

        expected_tag_size = len(_RSK_TAG) + _BLOCK_HEADER_HASH_SIZE
        if len(tx_tail[last_tag_pos:]) < expected_tag_size:
            message = "Last RSK tag '%s' found in tail '%s' is not long enough (expected at least %d bytes, got %d bytes)" % \
                (_RSK_TAG.hex(), tx_tail.hex(), expected_tag_size, len(tx_tail[last_tag_pos:]))
            _logger.info(message)
            raise ValueError(message)
        ...
        if len(remaining_tail) > _MAX_BYTES_AFTER_MERGE_MINING_HASH:
            message = "More than %d bytes after RSK tag" % (_MAX_BYTES_AFTER_MERGE_MINING_HASH)
            _logger.info(message)
            raise ValueError(message)

        byte_count = int.from_bytes(tx_midstate[:_BYTE_COUNT_LENGTH], byteorder='big', signed=False)
        coinbase_tx_length = byte_count+len(tx_tail)
        if coinbase_tx_length <= _MIN_COINBASE_TX_SIZE:
            message = "Coinbase transaction must be longer than %d bytes (got %d bytes)" % (_MIN_COINBASE_TX_SIZE, coinbase_tx_length)
            _logger.info(message)
            raise ValueError(message)

        merged_mining_hash = tx_tail[last_tag_pos+len(_RSK_TAG):last_tag_pos+len(_RSK_TAG)+_BLOCK_HEADER_HASH_SIZE].hex()
