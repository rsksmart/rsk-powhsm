...
class DongleAdmin:
    ...
    def handshake(self, master_key=None):
        ...
        # Identify
        self._send_command(self.CMD.IDENTIFY, bytes([0x00, 0x00, len(self.TARGET_ID)]) + self.TARGET_ID)

		# Exchange nonces
        nonce = os.urandom(self.NONCE_LENGTH)
        response = self._send_command(self.CMD.NONCE, bytes([0x00, 0x00, self.NONCE_LENGTH]) + nonce)
        device_nonce = response[4:12] # First 4 bytes are the device batch, we don't need it
        ...
        self._send_command(self.CMD.SEND_KEY, bytes([self.SUBCMD.SEND_KEY_MASTER, 0x00, len(certificate)]) + certificate)
        ...
        certificate = bytes([len(ephemeral_key_pub)]) + ephemeral_key_pub + \
                      bytes([len(signature)]) + signature
        self._send_command(self.CMD.SEND_KEY, bytes([self.SUBCMD.SEND_KEY_EPHEMERAL, 0x00, len(certificate)]) + certificate)
    ...
    def get_device_key(self):
        ...
        response = self._send_command(self.CMD.GET_KEY, bytes([self.SUBCMD.GET_KEY_DEVICE, 0x00, 0x00]))
        ...
        self._send_command(self.CMD.GET_KEY, bytes([self.SUBCMD.GET_KEY_EPHEMERAL, 0x00, 0x00]))
    ...
    def setup_endorsement_key(self, scheme, endorsement_certificate):
        ...
        # Expected signed data is: (sha256 of) endorsement role + endorsement public key
        signed_data = bytes([self.ROLE.ENDORSEMENT]) + endorsement_key_pub

        # Send endorsement certificate in order to confirm setup
        self._send_command(self.CMD.SETUP_ENDO_ACK, bytes([0x00, 0x00, len(endorsement_certificate)]) + endorsement_certificate)

        # Return the raw endorsement public key along with the raw signature and raw signed data
        return {
