...
To generate the attestation, the UI uses the configured attestation scheme to sign a message containing a predefined header (`RSK:HSM:UI:`) and the uncompressed custom certification authority's public key. As a consequence of the aforementioned features, this message guarantees that the device is running a specific version of the UI with a specific custom certification authority, and also that this cannot be changed without wiping the device, therefore losing the keys forever.
